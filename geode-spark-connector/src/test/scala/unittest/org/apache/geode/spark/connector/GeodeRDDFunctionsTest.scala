...
class GeodeRDDFunctionsTest extends FunSuite with Matchers with MockitoSugar {
  ...
  def verifyPairRDDFunction(useOpConf: Boolean): Unit = {
    ...
    result === Unit
    verify(mockSparkContext, times(1)).runJob(mockRDD, any[(TaskContext, Iterator[(String, String)]) => Unit])
  ...
  def verifyRDDFunction(useOpConf: Boolean): Unit = {
    ...
    result === Unit
    verify(mockSparkContext, times(1)).runJob(mockRDD, any[(TaskContext, Iterator[String]) => Unit])
    //   verify(mockSparkContext, times(1)).runJob[String, Unit](
    //   eq(mockRDD), any[(TaskContext, Iterator[String]) => Unit])(any(classOf[ClassTag[Unit]]))
