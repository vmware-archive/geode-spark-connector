...
public class GeodeJavaSQLContextFunctions {
    ...
    public Dataset<Row> geodeOQL(String query) {
        Dataset<Row> df = scf.geodeOQL(query, scf.defaultConnectionConf());
        return df;
    }

    public Dataset<Row> geodeOQL(String query, GeodeConnectionConf connConf) {
        Dataset<Row> df = scf.geodeOQL(query, connConf);
        return df;
    }
