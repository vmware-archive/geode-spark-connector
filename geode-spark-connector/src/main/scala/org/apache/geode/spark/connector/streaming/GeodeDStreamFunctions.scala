...
class GeodeDStreamFunctions[T](val dstream: DStream[T]) extends Serializable {
  ...
  def saveToGeode[K, V](
    ...
    LOG.info(s"""Save DStream region=$regionPath conn=${connConf.locators.mkString(",")}""")
...
/**
  ...
  * use these functions.
  */
class GeodePairDStreamFunctions[K, V](val dstream: DStream[(K, V)]) extends Serializable {
