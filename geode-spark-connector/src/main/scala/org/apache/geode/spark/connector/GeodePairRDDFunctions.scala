...
import org.apache.geode.spark.connector.internal.rdd.{GeodeJoinRDD, GeodeOuterJoinRDD, GeodePairRDDWriter}
...
class GeodePairRDDFunctions[K, V](val rdd: RDD[(K, V)]) extends Serializable {
  ...
  //TODO Need to think about the API with respect to the connection   Scala doesn't allow defaults in overloaded methods.
  ...
  /**
    ...
    */
  def saveToGeode(regionPath: String,
    ...
    LOG.info(s"""Save RDD id=${rdd.id} to region $regionPath""")
  ...
  /**
    ...
    * @return RDD[T, V]
    */
  def joinGeodeRegion[K2 <: K, V2](regionPath: String,
                                   connConf: GeodeConnectionConf): GeodeJoinRDD[(K, V), K, V2] = {
    if (connConf == null) {
      new GeodeJoinRDD[(K, V), K, V2](rdd, null, regionPath, defaultConnectionConf)
    } else {
      new GeodeJoinRDD[(K, V), K, V2](rdd, null, regionPath, connConf)
    }
  }

  def joinGeodeRegion[K2 <: K, V2](regionPath: String): GeodeJoinRDD[(K, V), K, V2] = {
    joinGeodeRegion(regionPath, defaultConnectionConf)
  }
  ...
  /**
    ...
    * @return RDD[(K, V), V2]
    */
  def joinGeodeRegion[K2, V2](regionPath: String,
                              func: ((K, V)) => K2,
                              connConf: GeodeConnectionConf): GeodeJoinRDD[(K, V), K2, V2] = {
    if (connConf != null) {
      new GeodeJoinRDD[(K, V), K2, V2](rdd, func, regionPath, connConf)
    } else {
      new GeodeJoinRDD[(K, V), K2, V2](rdd, func, regionPath, defaultConnectionConf)
    }
  }

  def joinGeodeRegion[K2, V2](regionPath: String,
                              func: ((K, V)) => K2): GeodeJoinRDD[(K, V), K2, V2] = {
    joinGeodeRegion(regionPath, func, defaultConnectionConf)
  }
  ...
  */
  ...
  /**
    ...
    */
  def outerJoinGeodeRegion[K2 <: K, V2](regionPath: String,
                                        connConf: GeodeConnectionConf): GeodeOuterJoinRDD[(K, V), K, V2] = {
    if (connConf != null) {
      new GeodeOuterJoinRDD[(K, V), K, V2](rdd, null, regionPath, connConf)
    } else {
      new GeodeOuterJoinRDD[(K, V), K, V2](rdd, null, regionPath, defaultConnectionConf)

    }
  }

  def outerJoinGeodeRegion[K2 <: K, V2](regionPath: String): GeodeOuterJoinRDD[(K, V), K, V2] = {
    outerJoinGeodeRegion(regionPath, defaultConnectionConf)
  }
  ...
  /**
    ...
    * @return RDD[ (K, V), Option[V] ]
    */
  def outerJoinGeodeRegion[K2, V2](regionPath: String,
                                   func: ((K, V)) => K2,
                                   connConf: GeodeConnectionConf): GeodeOuterJoinRDD[(K, V), K2, V2] = {
    if (connConf != null) {
      new GeodeOuterJoinRDD[(K, V), K2, V2](rdd, func, regionPath, connConf)
    } else {
      new GeodeOuterJoinRDD[(K, V), K2, V2](rdd, func, regionPath, defaultConnectionConf)
    }
  }

  def outerJoinGeodeRegion[K2, V2](regionPath: String,
                                   func: ((K, V)) => K2): GeodeOuterJoinRDD[(K, V), K2, V2] = {
    outerJoinGeodeRegion(regionPath, func, defaultConnectionConf)
  }

  /** This version of outerJoinGeodeRegion(...) is just for Java API. */
  /*
  private[connector] def outerJoinGeodeRegion[K2, V2](regionPath: String,
                                                      func: Function[(K, V), K2],
                                                      connConf: GeodeConnectionConf): GeodeOuterJoinRDD[(K, V), K2, V2] = {
